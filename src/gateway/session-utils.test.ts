...
describe("gateway session utils", () => {
  ...
  test("listAgentsForGateway does not surface per-agent voice on the core row", () => {
    // Voice lives in the SpriteCore plugin now. Clients read it from
    // GET /sprite-core/agents, not from the core gateway agent row.
    const cfg = {
      session: { mainKey: "main" },
      agents: {
        list: [{ id: "main", default: true }, { id: "ginger" }],
      },
    } as OpenClawConfig;

    const result = listAgentsForGateway(cfg);
    for (const agent of result.agents) {
      expect("voice" in agent).toBe(false);
    }
  });
